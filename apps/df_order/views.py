...
def order(request):
    ...
    coupons = Coupons_recive.objects.filter(buyer_id=int(uid)).filter(status=0)
    carts = []
    total_price = 0
    ...
    for coupon in coupons:
        lists.append(int(coupon.coupon_money))
    for i in lists:
        discount = discount + i
    final_price = total_trans_price-discount
...
def order_handle(request):
    ...
    coupons = Coupons_recive.objects.filter(buyer_id=int(user_id)).filter(status=0)
    data = {}
    umoney=0
    try:
...
def pay(request):
    uid = request.session['user_id']
    users=UserInfo.objects.get(uid)
    coupons = Coupons_recive.objects.filter(buyer_id=int(uid)).filter(status=0)
    total =0
    lists=[]
    for coupon in coupons:
        if lists.count(coupon.coupon_money)==0:
            lists.append(coupon.coupon_money)
    for i in lists:
        total = total+lists[i]
    return render(request,'df_order/place_order.html',context)